...
RUN apt update && \
	...
	apt install -y \
		...
		php php7.3-fpm php-curl php-date php-dom php-ftp php-gd php-iconv php-json \
...
RUN service mysql start && \
	...
	wp core install --allow-root --url="/"  --title="My Sexy Blog Title" --admin_user="admin" --admin_password="admin" --admin_email="admin@gmail.com" && \
...
# Expose http, https and mailserver ports
EXPOSE 80 443 110
